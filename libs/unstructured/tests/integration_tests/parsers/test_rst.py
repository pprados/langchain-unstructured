...
@pytest.mark.parametrize(
    "blob",
    couple_of_blob(EXAMPLE_DIRECTORY / "README.rst", FileType.RST.mime_type),
)
@pytest.mark.parametrize(
    "kwargs,doc_lens",
    [
...
@pytest.mark.local
def test_unstructured_rst_parser(kwargs: Any, doc_lens: int, blob: Blob) -> None:
    """Test unstructured loader."""
    parser = UnstructuredRSTParser(**kwargs)
...
def test_unstructured_rst_parser_via_api(blob: Blob) -> None:
    """Test unstructured loader."""
    parser = UnstructuredRSTParser(mode="single", partition_via_api=True)
    docs = list(parser.lazy_parse(blob))
