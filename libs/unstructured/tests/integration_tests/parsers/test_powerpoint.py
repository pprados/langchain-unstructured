...
@pytest.mark.parametrize(
    "blob",
    couple_of_blob(EXAMPLE_DIRECTORY / "README.pptx", FileType.PPTX.mime_type),
)
@pytest.mark.parametrize(
    "kwargs,doc_lens",
    [
...
@pytest.mark.local
def test_unstructured_powerpoint_parser(kwargs: Any, doc_lens: int, blob: Blob) -> None:
    """Test unstructured loader."""
    parser = UnstructuredPowerPointParser(**kwargs)
...
@pytest.mark.parametrize(
    ...
    (
        couple_of_blob(EXAMPLE_DIRECTORY / "README.pptx", FileType.PPTX.mime_type)
        + couple_of_blob(EXAMPLE_DIRECTORY / "README.ppt", FileType.PPT.mime_type)
    ),
)
def test_unstructured_excel_parser_via_api(blob: Blob) -> None:
    """Test unstructured loader."""
    parser = UnstructuredPowerPointParser(mode="single", partition_via_api=True)
    docs = list(parser.lazy_parse(blob))
