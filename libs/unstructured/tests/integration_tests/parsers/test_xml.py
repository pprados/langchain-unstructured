...
@pytest.mark.parametrize(
    "blob",
    couple_of_blob(EXAMPLE_DIRECTORY / "factbook.xml", FileType.XML.mime_type),
)
@pytest.mark.parametrize(
    "kwargs,doc_lens",
    [
...
@pytest.mark.local
def test_unstructured_xml_parser(kwargs: Any, doc_lens: int, blob: Blob) -> None:
    """Test unstructured loader."""
    parser = UnstructuredXMLParser(**kwargs)
...
def test_unstructured_xml_parser_via_api(blob: Blob) -> None:
    """Test unstructured loader."""
    parser = UnstructuredXMLParser(mode="single", partition_via_api=True)
    docs = list(parser.lazy_parse(blob))
