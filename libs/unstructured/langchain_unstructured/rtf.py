...
from langchain_unstructured.parsers.rtf import UnstructuredRTFParser
from langchain_unstructured.unstructured import _UnstructuredBaseLoader


class UnstructuredRTFLoader(_UnstructuredBaseLoader):
    ...
    Setup:
        Install ``langchain-unstructured[rtf]``.

        .. code-block:: bash

            pip install -U langchain-unstructured[rtf].

    Instantiate:
        .. code-block:: python

            from langchain_community import UnstructuredRTFLoader
    ...
    def lazy_load(
        ...
        blob = Blob(path=self.file_path, mimetype=FileType.RTF.mime_type)
