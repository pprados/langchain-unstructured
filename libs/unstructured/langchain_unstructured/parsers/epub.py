...
class UnstructuredEPubParser(_UnstructuredDocumentParser):
    """Parse `EPub` blob using `Unstructured`.
    ...
    Setup:
        Install ``langchain-unstructured[epub]``.

        .. code-block:: bash

            pip install -U langchain-unstructured[epub]
    ...
    Instantiate:
        ...
        .. code-block:: python
            ...
            blob=Blob.from_path("example.epub")
            parser = UnstructuredEPubParser(
                mode="single",
            )
    ...
    def __init__(
        ...
        *,
        ...
        mode: Literal["elements", "single", "paged", "page"] = DEFAULT_EPUB_MODE,
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        ...
        assert (
            blob.mimetype == FileType.EPUB.mime_type
            or blob.mimetype in FileType.EPUB._alias_mime_types
        ), f"Invalide mime-type {blob.mimetype}"
        partition_fn = partition_epub
        if blob.data is None and blob.path:
            ...
            with blob.as_bytes_io() as file:
                return partition_fn(
                    filename=None, file=file, **self.unstructured_kwargs
                )
