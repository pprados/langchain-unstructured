...
class UnstructuredHTMLParser(_UnstructuredDocumentParser):
    """Parse `HTML` blob using `Unstructured`.
    ...
    Setup:
        Install ``langchain-unstructured[html]``.

        .. code-block:: bash

            pip install -U langchain-unstructured[html]
    ...
    Instantiate:
        ...
        .. code-block:: python
            ...
            blob=Blob.from_path("example.html")
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        ...
        assert blob.mimetype == FileType.HTML.mime_type, (
            f"Invalide mime-type {blob.mimetype}"
        )
        partition_fn = partition_html
