...
class UnstructuredCSVParser(_UnstructuredDocumentParser):
    """Parse `CSV` blob using `Unstructured`.

    Like other
    Unstructured parser, UnstructuredCSVParser can be used in both
    "single" and "elements" mode. If you use the parser in "elements"
    mode, the CSV blob will be a single Unstructured Table element.
    If you use the loader in "elements" mode, an HTML representation
    of the table will be available in the "text_as_html" key in the
    document metadata.

    Setup:
        Install ``langchain-unstructured[csv]``.

        .. code-block:: bash

            pip install -U langchain-unstructured[csv]
    ...
    Instantiate:
        .. code-block:: python
            ...
            blob=Blob.from_path("example.csv")
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        ...
        assert blob.mimetype == FileType.CSV.mime_type, (
            f"Invalide mime-type {blob.mimetype}"
        )

        partition_fn = partition_csv
