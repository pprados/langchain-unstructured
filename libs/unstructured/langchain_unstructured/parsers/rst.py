...
class UnstructuredRSTParser(_UnstructuredDocumentParser):
    """Parse `RST` blob using `Unstructured`.
    ...
    Instantiate:
        ...
        .. code-block:: python
            ...
            blob=Blob.from_path("example.rst")
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        try:
            ...
            raise ImportError(
                ...
                "`pip install 'langchain_unstructured[pandoc]'`"
            ) from e

        assert blob.mimetype == FileType.RST.mime_type, (
            f"Invalide mime-type {blob.mimetype}"
        )
        partition_fn = partition_rst
