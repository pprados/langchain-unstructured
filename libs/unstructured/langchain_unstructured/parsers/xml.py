...
class UnstructuredXMLParser(_UnstructuredDocumentParser):
    """Parse `XML` blob using `Unstructured`.
    ...
    Setup:
        Install ``langchain-unstructured[xml]``.

        .. code-block:: bash

            pip install -U langchain-unstructured[xml]

    Instantiate:
        .. code-block:: python

            from langchain_community import UnstructuredXMLLoader
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        ...
        assert blob.mimetype == FileType.XML.mime_type, (
            f"Invalide mime-type {blob.mimetype}"
        )
        partition_fn = partition_xml
