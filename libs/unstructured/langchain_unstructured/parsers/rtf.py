...
class UnstructuredRTFParser(_UnstructuredDocumentParser):
    """Parse `RTF` blob using `Unstructured`.
    ...
    Instantiate:
        ...
        .. code-block:: python
            ...
            blob=Blob.from_path("example.rtf")
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        try:
            ...
            raise ImportError(
                ...
                "`pip install 'langchain_unstructured[rtf]'`"
            ) from e

        assert blob.mimetype == FileType.RTF.mime_type, (
            f"Invalide mime-type {blob.mimetype}"
        )
        partition_fn = partition_rtf
