...
class UnstructuredMarkdownParser(_UnstructuredDocumentParser):
    """Parse `Markdown` blob using `Unstructured`.
        ...
        Setup:
            Install ``langchain-unstructured[markdown]``.

            .. code-block:: bash

                pip install -U langchain-unstructured[markdown]

    Instantiate:

        .. code-block:: python
            from langchain_core.documents.base import Blob
            from langchain_unstructured.parsers import UnstructuredMarkdownParser

            blob=Blob.from_path("example.md")
            parser = UnstructuredMarkdownParser(
                mode="single",
            )
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        try:
            ...
            raise ImportError(
                ...
                "`pip install 'langchain_unstructured[md]'`"
            ) from e

        assert blob.mimetype == FileType.MD.mime_type, (
            f"Invalide mime-type {blob.mimetype}"
        )
        partition_fn = partition_md
