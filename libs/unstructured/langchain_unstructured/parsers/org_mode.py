...
class UnstructuredOrgModeParser(_UnstructuredDocumentParser):
    """Parse `Org-Mode` blob using `Unstructured`.
    ...
    Setup:
        Install ``langchain-unstructured[org]``.

        .. code-block:: bash

            pip install -U langchain-unstructured[org]

    Instantiate:

        .. code-block:: python
            from langchain_core.documents.base import Blob
            from langchain_unstructured.parsers import UnstructuredOrgParser

            blob=Blob.from_path("example.org")
            parser = UnstructuredOrgParser(
                mode="single",
            )
    ...
    def _get_elements_via_local(self, blob: Blob) -> list:
        try:
            ...
            raise ImportError(
                ...
                "`pip install 'langchain_unstructured[org]'`"
            ) from e

        assert blob.mimetype == FileType.ORG.mime_type, (
            f"Invalide mime-type {blob.mimetype}"
        )
        partition_fn = partition_org
